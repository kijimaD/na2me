...
func TestSplitByPeriod(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			forceLen:  20,
			expect: []string{
				"「王様は九冊が六冊になったから少しは価も減ったろうと思って六冊でいくらだと聞くと、",
				"やはり元の通り一文も引かないそうです、それは乱暴だと云うと、", "その女はまた三冊をとって火にくべたそうです。王様はまだ未練があったと見えて、",
				"余った三冊をいくらで売ると聞くと、やはり九冊分のねだんをくれと云うそうです。九冊が六冊になり、",
				"六冊が三冊になっても代価は、元の通り一厘も引かない、",
				"それを引かせようとすると、残ってる三冊も火にくべるかも知れないので、",
				"王様はとうとう高い御金を出して焚け余りの三冊を買ったんですって……どうだこの話しで少しは書物のありがた味が分ったろう、",
				"どうだと力味むのですけれど、私にゃ何がありがたいんだか、",
				"まあ分りませんね」"},
		},
	}
